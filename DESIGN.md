...
## 5.6 全域 system prompt（輸出格式指示）

owner 要「每則 Ollama 回覆都自動有固定格式」（`# 標題` 開頭、結尾附 `#### Key words`／`#### Tags`）。
做法是**全域可編輯的 system prompt**——不是硬寫在程式碼、也不是每次自己在 prompt 裡打。

- **儲存**：全域單檔 `settings.json`（`{ systemPrompt }`），比照 `prompts.json` 的 owner-registry
  式覆寫（覆寫前 `.bak`，寫入頻率低）。**無檔時 `GET /settings` 回一段預設格式指示**（＝開箱即用，
  且 GET 不落地寫檔，避免「讀順手寫」）；空字串＝停用（不 prepend 任何 system 訊息）。
- **注入點在前端、送出時組裝、不落地**：`withSystemPrompt(messages, systemPrompt)`＝`flattenForApi()`
  再把非空的 systemPrompt `unshift` 成一則 `role:'system'` 訊息。只在呼叫 `/chat` 前組裝，**不寫進
  turn 結構**（system prompt 是全域設定、不是某則對話的內容；這和 §1「flatten 只在送出時做」同精神）。
  後端 `/chat` 仍是純 proxy，不碰 settings。
- **與自動命名（§5.1）獨立**：`/title` 端點有自己的命名 system prompt，不受這個全域格式指示影響。
- **老實說是「盡量遵守」非保證**：本機小模型（qwen2.5）多半會照做（實測 14b 對「標題＋Key words＋
  Tags」遵守良好），但長對話、極短回覆時偶爾會漏或跑掉——這是模型行為，不是機制問題。
- **資料格式早已預留**：`TURN_ROLE_WHITELIST`／扁平信封白名單本來就含 `system`（原 §6「UI 未出」
  的那筆，現在補出來了）。未來要放 temperature 等參數，也擴充在 `settings.json` 同一個面。

## 6. v1 刻意不做

- **語法上色（highlight.js）**：先有複製鈕；要加時注意主題同步（github-dark ↔ github）。
- **per-conversation system prompt／參數（temperature 等）**：目前 system prompt 是**全域**單一份（§5.6）；
  每組對話各自的 system prompt／參數（temperature…）尚未做，要時擴充 `settings.json` 或改存進 chat。
